...
@section('page_title', 'Tambah Ketua RT')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
        <div class="mb-6">
            <h3 class="text-lg font-bold text-slate-800">Formulir Tambah Ketua RT</h3>
            <p class="text-xs text-slate-400 mt-1">Lengkapi data Ketua RT di bawah ini. Pastikan nomor WhatsApp dalam format Fonnte (diawali 62).</p>
        </div>
        ...
        <form action="{{ route('ketua-rt.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Nama Lengkap -->
            <div>
                <label for="nama" class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Nama Lengkap</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required placeholder="Nama Lengkap Ketua RT"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="rt" id="rt" value="{{ old('rt') }}" required maxlength="3" placeholder="001"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-400">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="rw" id="rw" value="{{ old('rw') }}" required maxlength="3" placeholder="001"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-400">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="text" name="no_whatsapp" id="no_whatsapp" value="{{ old('no_whatsapp') }}" required placeholder="628123456789"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-400">
                </div>
